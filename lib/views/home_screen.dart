            ...
            return GestureDetector(
              onTap: () {
                ...
                switch (route) {
                  ...
                  // 1. Dành cho SINH VIÊN: Gửi và xem lịch sử đơn của mình
                  case '/diem_danh_sv':
                    Navigator.push(
                      context,
                      MaterialPageRoute(
                        // widget.studentId là mã của người dùng hiện tại đang đăng nhập
                        builder: (context) => DiemDanhSvScreen(studentId: widget.studentId),
                      ),
                    );
                    break;
                  case '/mo_diem_danh':
                    Navigator.push(
                      context, 
                      MaterialPageRoute(builder: (context) => MoDiemDanhScreen(lecturerId: widget.studentId))
                    );
                    break;
                  case '/quan_ly_nghi_hoc':
                    Navigator.push(
                      context,
                      MaterialPageRoute(
                        builder: (context) => QuanLyXinPhepScreen(studentId: widget.studentId),
                      ),
                    );
                    break;

                  // 2. Dành cho CÁN BỘ: Danh sách đơn của SV gửi đến để Duyệt
                  case '/duyet_vang_hoc':
                    Navigator.push(
                      context,
                      MaterialPageRoute(
                        builder: (context) => DuyetVangHocScreen(lecturerId: widget.studentId), // widget.studentId lúc này là mã CB
                      ),
                    );
                    break;
                  case '/chatscreen':
