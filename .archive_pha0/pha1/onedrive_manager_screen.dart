        ...
        : _items.isEmpty 
          ? FutureBuilder<SharedPreferences>(
              ...
              builder: (context, snapshot) {
                final token = snapshot.data?.getString('ms_access_token');
                final expiry = snapshot.data?.getInt('ms_token_expiry');
                bool isExpired = expiry == null || DateTime.now().millisecondsSinceEpoch > expiry;
                
                // Nếu token null HOẶC đã hết hạn, hiện bảng bắt login
                if (token == null || token.isEmpty || isExpired) return _buildLoginRequiredState();
