...
void _processLoginUri(Uri uri) async {
  ...
  // 1. Xác thực máy trước (FaceID/Vân tay/Passcode)
  final isDeviceOwner = await _authService.authenticateWithDevice();
  if (!isDeviceOwner) return;

  // 2. Kiểm tra quyền Camera ngay tại đây
  var status = await Permission.camera.status;
  if (status.isDenied) {
    status = await Permission.camera.request();
  }

  // 3. Nếu người dùng từ chối quyền Camera
  if (status.isPermanentlyDenied || status.isDenied) {
    _showErrorSnackBar("Bạn cần cấp quyền Camera để thực hiện nhận diện khuôn mặt!");
    return; // Dừng lại, không push sang màn hình tiếp theo
  }

  if (!mounted) return;
  
  // 4. Chỉ khi có quyền mới mở màn hình quét
  final result = await Navigator.push(
    context,
    MaterialPageRoute(builder: (context) => const FaceScanProScreen()), 
  );

  // Xử lý kết quả trả về (giữ nguyên logic cũ của bạn)
  if (result != null && result['front'] != null) {
    ...
    if (aiResult['status'] == 'SUCCESS') {
      ...
      _showErrorSnackBar(aiResult['message'] ?? "Không nhận diện được khuôn mặt!");
    }
