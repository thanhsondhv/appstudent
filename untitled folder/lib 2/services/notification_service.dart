...
static void _handleNavigation(Map<String, dynamic> data) {
  ...
  // Sử dụng navigatorKey để điều hướng mà không cần context
  ...
  Future<void> syncTokenToServer(String userId) async {
    try {
      ...
      if (prefs.getString('last_fcm_token') == token) return;

      final response = await http.post(
        Uri.parse("$domainApi/save-fcm-token"),
        headers: {"Content-Type": "application/json"},
        body: jsonEncode({
          "student_id": userId, // Gửi nguyên bản, Backend sẽ tự clean
          "token": token,
      ...
      if (response.statusCode == 200) {
        ...
        debugPrint("✅ Đã đồng bộ Token cho $userId");
      }
    } catch (e) {
      debugPrint("⚠️ Lỗi Sync Token: $e");
    }
  }
  ...
  static Future<List<dynamic>> fetchAndSyncNotifs(String userId) async {
    ...
    try {
      ...
      debugPrint("🔥 Lỗi đồng bộ tin nhắn: $e");
    }
    return localData;
  }
}