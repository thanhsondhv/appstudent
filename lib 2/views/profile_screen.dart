...
class _ProfileScreenState extends State<ProfileScreen> {
  ...
  Future<void> _loadProfile() async {
    ...
    // 1. Lấy vai trò người dùng (Sửa từ 'role' thành 'user_role' để đồng bộ)
    String role = prefs.getString('user_role') ?? "SinhVien"; 
    
    // 2. Lấy mã ID (ưu tiên user_code)
    String rawId = prefs.getString('user_code') ?? prefs.getString('user_id') ?? "";
    
    // 3. Xử lý cắt bỏ tiền tố (SV hoặc CB) để lấy mã số thuần túy
    String cleanId = rawId.toUpperCase().replaceAll(RegExp(r'^(SV|CB)'), ''); 

    setState(() {
      studentName = prefs.getString('full_name') ?? "Người dùng";
      studentId = cleanId;
      userRole = role; // Gán đúng vai trò "CanBo" hoặc "SinhVien" vào state
      isNotifEnabled = prefs.getBool('receive_notifications') ?? true;
    });

    // 4. Chỉ gọi API lấy thông tin lớp/trạng thái nếu có ID
    if (cleanId.isNotEmpty) {
      _fetchStudentExtraInfo(cleanId);
    }
    
    debugPrint("✅ Profile Loaded: ID=$studentId, Role=$userRole");
  }
